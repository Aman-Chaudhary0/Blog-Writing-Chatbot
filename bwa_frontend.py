...
def strip_markdown_code_blocks(md: str) -> str:
    # Remove fenced code blocks first (```...```), then indented code blocks.
    no_fenced = re.sub(r"```[\s\S]*?```", "", md)
    no_indented = re.sub(r"(^|\n)(?: {4}|\t).*(?=(\n|$))", "", no_fenced)
    return no_indented
...
with st.sidebar:
    ...
    show_code_blocks = st.checkbox("Show code blocks in preview", value=False)
...
if out:
    ...
    with tab_preview:
        ...
        if not final_md:
            ...
            if not show_code_blocks:
                final_md = strip_markdown_code_blocks(final_md)
